...
## Library (using npm)
Golden Layout used to be shipped via NPM. However, be warned the NPM modules have not been updated in a long time, so [building from source](#source) is currently recommended. Use the following commands to install it into an application package:\
`npm i golden-layout`
